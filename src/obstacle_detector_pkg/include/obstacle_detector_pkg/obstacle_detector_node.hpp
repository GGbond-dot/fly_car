...
struct Polyline
{
  ...
  double           perp_dist = 0.0;     // 车（laser 原点）到折线最近一段的垂直距离
};
...
private:
  ...
  // 把有序点按邻近断点切成若干点链，取点数最多的一条作为墙
  std::vector<Pt2> longestChain(const std::vector<Pt2> & pts) const;
  // Split-and-Merge 折线拟合（含近共线段合并）
  Polyline fitPolyline(const std::vector<Pt2> & chain, const Pt2 & car_xy) const;
