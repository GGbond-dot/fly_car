    ...
    return LaunchDescription([
        ...
        Node(
            ...
            parameters=[{
                ...
                # kp_w=0.6:平顺优先取向。曾提到 0.9 治角点外扩,但角点只好一点点(还剩6~7cm)、
                ...
                'v_min_mps': 0.14,
                # v_min 地板方位门:|e_h|<此角(直行)才兜底防死区;急拐放开 → v 贴 cos 降速原地拧,角点半径变小不外扩。
                'v_floor_gate_deg': 35.0,
                # 转向前馈:补左轮偏快的恒定漂移(开环实测 v=0.2 时 Ω=-0.026rad/s,半径~8m)。
                # ⚠ 实测 0.025 会把车从"歪线外4.4cm"拉到贴线,但贴线后 carto 噪声被 kp_w=0.9 放大成画龙
                #   (直边 yaw_rate std 18.7→34.2)。权衡后设 0=不补,保平顺、接受直边~4cm 外扩(圆角本身也没多少直段)。
                #   若想两全:先把 kp_w 降到 ~0.7 压噪声再开 w_bias,需另调。
                'w_bias_rps': 0.0,
