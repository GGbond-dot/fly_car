...
3. 按相邻点距离 `chain_break_dist_m` 断链，取**点数最多的一条链**作为墙（墙是连续的）。
...
7. 有效则每帧连续发布到 `/detected_obstacle`。
...
| 2N+1 | perp_dist：车到折线最近一段的垂直距离 |
| 2N+2 | total_length：折线总长 |

> 顶点串完整表达折线形状，下游可自取：跨越点、墙的走向/拐点、到墙距离等，不做限制。
...
- 整体目标是"平地遍历地图 → 遇障起飞"：检测节点持续输出折线墙，由上层任务节点用 `perp_dist` 判断"逼近到墙前阈值"→ 触发起飞越障。
